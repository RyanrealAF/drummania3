...
const PlayableStem = ({ label, url, playSound }: { label: string, url: string, playSound: (url: string) => void }) => (
    <TouchableOpacity onPress={() => playSound(url)}>
        <Text style={styles.stemLink}>Play {label}</Text>
    </TouchableOpacity>
);
...
export default function SequencerScreen() {
    ...
    const getStatusMessage = () => {
        switch (processingStatus) {
            ...
            case 'done': return 'Processing complete! Stems saved locally.';
    ...
    return (
        <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
            ...
            <View style={[styles.mainContent, processingStatus !== 'idle' && processingStatus !== 'done' && styles.blurred]}>
                ...
                {stems && processingStatus === 'done' && (
                    <View style={styles.stemsLinks}>
                        <Text style={styles.stemsTitle}>Play Separated Stems</Text>
                        {Object.entries(stems).map(([key, value]) => {
                            if (typeof value === 'string') {
                                return <PlayableStem key={key} label={key} url={value} playSound={playSound} />;
                            }
                            return null;
                        })}
                    </View>
...
const styles = StyleSheet.create({
    ...
    stemsLinks: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-around', width: '100%', backgroundColor: '#1a1a1a', padding: 15, borderRadius: 15, marginBottom: 20 },
    stemsTitle: { color: 'white', fontSize: 14, fontWeight: 'bold', textAlign: 'center', width: '100%', marginBottom: 10 },
    stemLink: { color: '#00aaff', fontSize: 14, padding: 8, textDecorationLine: 'underline' },
